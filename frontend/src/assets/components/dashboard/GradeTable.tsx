    ...
    <div className="overflow-x-auto">
      <table className="w-full text-left text-sm border-separate border-spacing-0">
        <thead className="bg-slate-50">
          <tr>
            <th className="p-4 font-bold text-slate-400 text-[10px] tracking-widest uppercase border-b">
              Subject
            </th>
            <th className="p-4 font-bold text-slate-400 text-[10px] tracking-widest uppercase border-b text-center">
              Score
            </th>
            <th className="p-4 font-bold text-slate-400 text-[10px] tracking-widest uppercase border-b text-center">
              Grade
            ...
            <th className="p-4 font-bold text-slate-400 text-[10px] tracking-widest uppercase border-b text-right">
              Class Avg
            </th>
          </tr>
        </thead>
          ...
          {safeGrades.map((item, idx) => {
            ...
            return (
              <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="p-4 text-center">
                  <span
                    className={`inline-flex items-center justify-center w-8 h-8 rounded-lg font-black shadow-sm ${getGradeColor(item.grade_letter)}`}
                  >
                    {item.grade_letter}
                  </span>
                </td>
                ...
                <td className="p-4 text-center">
                  <span
                    className={`px-2.5 py-1 rounded-full text-[11px] font-bold border ${status.color}`}
                  >
                    {status.label}
                  </span>
                </td>
