      ...
      {students.length <= 0 ? (
        <div className="bg-white border border-slate-200 rounded-3xl p-10 text-center shadow-sm">
          ...
          <p className="mt-2 text-xs text-slate-400 max-w-xs mx-auto">
            There are currently no student records available. Add a student to
            get started.
          </p>
        </div>
      ) : (
                ...
                <td className="px-6 py-4">
                  ...
                  >
                    <Image
                      ...
                      height={100}
                      alt="description"
                      src={`https://ui-avatars.com/api/?name=${encodeURIComponent(student.fullName)}&background=random&color=fff&rounded=true&size=128`}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-[10px] font-bold flex items-center gap-2 w-fit ${styles[status]}`}
    >
