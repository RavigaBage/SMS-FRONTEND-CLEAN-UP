...
export interface ImportModalConfig {
  ...
  transformPayload: (row: Record<string, any>) => Record<string, any>;
}
...
export function ImportModal({ isOpen, onClose, onSuccess, file, config }: ImportModalProps) {
  ...
  const runImport = useCallback(async () => {
    ...
    try {
      ...
      const initialRows: RowResult[] = rawRows.map((r, i) => ({
        index: i,
        name: config.rowLabel(r) || `Row ${i + 2}`,
        status: "pending",
      }));
      setRows(initialRows);
    } catch {
      setHeaderError("Failed to read the file. Please check the format.");
      setStage("idle");
      return;
    }
    ...
    for (let i = 0; i < rawRows.length; i++) {
      ...
      try {
        const payload = config.transformPayload(rawRows[i]);
            ...
            <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-100 rounded-2xl">
              ...
              <div>
                ...
                <p className="text-[11px] text-red-400 mt-1">
                  Required: {config.requiredHeaders.join(", ")}
                </p>
              </div>
            </div>
          )}
          ...
          {stage === "idle" && !headerError && (
            <div className="flex flex-col items-center justify-center gap-3 py-6">
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-slate-400 mt-1">Ready to extract and upload records</p>
              </div>
            </div>
          )}
        ...
        return (
          <div key={s.key} className="flex-1 flex flex-col items-center gap-1">
            <div className={`w-full h-1 rounded-full transition-all duration-500 ${
              isDone ? "bg-emerald-400" : isActive ? "bg-cyan-500" : "bg-slate-100"
            }`} />
            <span className={`text-[9px] font-black uppercase tracking-widest ${
              isDone ? "text-emerald-500" : isActive ? "text-cyan-600" : "text-slate-300"
            }`}>
              {s.label}
            </span>
          </div>
...
function RowItem({ row }: { row: RowResult }) {
  return (
    <div className={`flex items-center gap-3 px-4 py-2.5 rounded-xl border transition-all duration-300 ${
      ...
      : row.status === "uploading" ? "bg-cyan-50 border-cyan-200"
      ...
      <span className={`text-[9px] font-black uppercase tracking-widest shrink-0 ${
        ...
        : row.status === "uploading" ? "text-cyan-500"
        : "text-slate-300"
      }`}>
        {row.status}
      </span>
