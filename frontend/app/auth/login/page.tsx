...
export default function LoginPage() {
  ...
  // 🔹 Fetch public JWT on load
  useEffect(() => {
    const fetchJwtToken = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/get_jwt_token_by_api_key/?api_key=MY_SUPER_SECRET_KEY`,
          { method: "GET" }
        );
        ...
        if (res.ok && data?.responseCode === 0) {
          ...
          console.error("JWT error:", data?.responseMessage);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.responseCode === 0) {
        ...
        document.cookie = `token=${data.data.access}; path=/`;

        
      } else {
      
        setResponse({"status":"error","message":"An error occurred try again !"});
