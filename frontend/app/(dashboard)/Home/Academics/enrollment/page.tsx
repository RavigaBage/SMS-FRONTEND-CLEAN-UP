      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Track and manage student enrollments across academic years.
            </p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            {
              label: "Active",
              value: enrollmentData.filter(
                (e) => e.status_display.toLowerCase() === "active",
              ).length,
        ...
        {isSearching && searchQuery && (
          <div className="flex items-center gap-2 text-sm text-slate-500">
            ...
            <button
              onClick={resetSearch}
              className="ml-1 text-xs font-bold text-rose-500 hover:underline"
            >
              Clear
            </button>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-visible">
                ...
                <tr>
                  {[
                    ...
                    (h) => (
                      <th
                        key={h}
                        className="px-6 py-3.5 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ),
          ...
          {!isLoading && enrollmentData.length > 0 && (
            <div className="px-6 py-3 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between text-xs text-slate-400">
              <span>
                Showing {enrollmentData.length} of {pagination?.count ?? 0}{" "}
                enrollments
              </span>
              {pagination && (
                <Pagination
