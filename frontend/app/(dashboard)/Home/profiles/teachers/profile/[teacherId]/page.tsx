...
export default function teacherProfilePage({
  ...
  const fetchteacherDetails = async () => {
    setLoading(true);
    try {
      const data = await apiRequest(`/teacherProfile/${teacherId}/`, { method: "GET" });
  ...
  if (!currentteacher)
    return (
      <div className="p-20 text-center bg-slate-50 min-h-screen">
        <p className="text-slate-500 font-bold">teacher member not found.</p>
        <Link
          href="/management/teach"
          className="text-cyan-600 hover:underline mt-4 block text-sm"
        >
          Return to Directory
        </Link>
      </div>
      ...
      <div className="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-slate-200 mb-8">
        ...
        <div className="px-8 pb-8 flex flex-col md:flex-row items-end gap-6 -mt-12">
          <div className="relative">
            <Image
              src={`https://api.dicebear.com/7.x/initials/svg?seed=${currentteacher.first_name}`}
              className="w-32 h-32 rounded-[2rem] border-8 border-white bg-slate-100 shadow-sm object-cover"
              alt="teacher Profile"
              height={1}
              width={1}
            />
            {currentteacher.user?.is_active && (
          ...
          <div className="flex-1 pb-2">
            ...
            <p className="text-slate-500 font-bold text-xs uppercase tracking-wider flex items-center gap-2">
              USERNAME: {currentteacher.full_name} •{" "}
              {currentteacher.teacher_type_display} •{" "}
              {currentteacher.specialization || "General"}
            </p>
          </div>
          <div className="flex gap-3 pb-2">
            <a
              href={`mailto:${currentteacher.email}`}
              className="flex items-center gap-2 px-5 py-2.5 bg-slate-50 text-slate-700 font-bold rounded-xl border border-slate-200 hover:bg-slate-100 transition-all text-xs uppercase"
            >
              <Mail size={16} /> Email
            </a>
            <button
              onClick={() => setIsEditModalOpen(true)}
              className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-all shadow-lg shadow-slate-200 text-xs uppercase"
            >
        ...
        <div className="col-span-12 lg:col-span-4 space-y-8">
          <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
            <h3 className="flex items-center gap-2 text-slate-800 font-black uppercase tracking-[0.2em] text-[10px] mb-8">
              <GraduationCap size={16} className="text-cyan-500" /> Professional
              Details
            </h3>
            <div className="space-y-6">
              <DetailItem
                icon={<Calendar />}
                label="Date of Birth"
                value={currentteacher.date_of_birth || "Not Provided"}
              />
              <DetailItem
                icon={<Phone />}
                label="Phone Number"
                value={currentteacher.phone_number || "Not Provided"}
              />
              <DetailItem
                icon={<Mail />}
                label="Work Email"
              ...
              <DetailItem
                icon={<MapPin />}
                label="Address"
                value={currentteacher.address || "No address on file"}
              />
              <DetailItem
                icon={<Briefcase />}
                label="Joined School"
                value={currentteacher.employment_date || "Not Provided"}
              />
              <DetailItem
                icon={<Fingerprint />}
                label="National ID"
                value={currentteacher.national_id || "Pending Verification"}
              />
            </div>
            ...
            <h3 className="flex items-center gap-2 text-slate-800 font-black uppercase tracking-[0.2em] text-[10px] mb-6">
              <HeartPulse size={16} className="text-rose-500" /> Medical /
              Health
            </h3>
            <p className="text-sm text-slate-600 font-medium bg-rose-50/50 p-4 rounded-2xl border border-rose-100 italic">
              {currentteacher.health_info ||
                "No critical health information recorded."}
            </p>
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-8 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <StatCard
              ...
              value={currentteacher.user?.is_active ? "Active" : "Inactive"}
              trend="System Verified"
              icon={<Star size={18} className="text-amber-400" />}
            />
            <StatCard
              label="Joined"
              value={new Date(currentteacher.created_at).getFullYear().toString()}
              trend="Registry Year"
              icon={<Clock size={18} className="text-cyan-500" />}
            />
            <StatCard
              label="Gender"
              value={currentteacher.gender_display}
              trend="Biometric Data"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6">
              ...
              <div className="space-y-3">
                {currentteacher?.managed_classes?.length > 0 ? (
                  currentteacher.managed_classes.map((cls: any) => (
                    <div
                      key={cls.id}
            ...
            <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6">
              ...
              <div className="space-y-3">
                {currentteacher?.assigned_subjects?.length > 0 ? (
                  currentteacher.assigned_subjects.map((sub: any) => (
                    <div
                      key={sub.id}
                      className="group flex items-center gap-4 p-4 hover:bg-slate-50 rounded-2xl transition-colors border border-transparent hover:border-slate-100"
                    >
